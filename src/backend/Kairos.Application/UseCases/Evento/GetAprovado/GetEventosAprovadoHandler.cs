namespace Kairos.Application.UseCases.Evento.GetAprovado;
public class GetEventosAprovadoHandler(IEventoRepository repository)
{
    public async Task<PagedList<List<GetEventosResponse>?>> GetAprovadoHandler(GetEventosCommand command, CancellationToken token)
    {
        try
        {
            var response = await repository.GetEventosAprovadosAsync(command,token);
